...
class VRP_Problem:
    ...
    def restricted_labels_check(self):
        ...
        if self.test3_restricted_labels.get((tuple(sorted(self.new_visited)), self.new_last_point)) == None:
            ...
            if self.new_time < self.test3_restricted_labels.get((tuple(sorted(self.new_visited)), self.new_last_point)):
                logging.debug(
                    f"{self.new_time} < {self.test3_restricted_labels.get((tuple(sorted(self.new_visited)), self.new_last_point))}")
                logging.info(f"({self.new_visited}, {self.new_last_point}, {self.new_time}) IS NOT on restricted list")
                test3 = True
            else:
                logging.debug(
            ...
            if self.new_time <= self.LDT_array[x][self.new_last_point]:
                ...
                for y in check2:
                    ...
                    if new_time1 > self.LDT_array[y][new_last_point1]:
                        ...
                        new_last_point2 = y
                        new_time2 = new_time1 + self.service_times[new_last_point1] + \
                                    self.travel_times_array[new_last_point1][y]
    ...
    def dominance_test(self):
        ...
        if len({k: v for k, v in self.memo.items() if k[0] == self.new_visited and k[
            1] == self.new_last_point}) != 0:  # possible dominated situation
            if (self.new_time <= self.first) and (
                ...
                logging.info(
                    f"label ({self.new_visited}, {self.new_last_point}, {self.new_time}) dominated, case 1, replaces label ({self.new_visited},{self.new_last_point},{self.first})")
            elif (self.new_time == self.first) and (
                    self.new_dist < self.memo[(self.new_visited, self.new_last_point, self.first)][
                ...
                logging.info(
                    f"label ({self.new_visited}, {self.new_last_point}, {self.new_time}) dominated, case 5, no label created")
    ...
    def retrace_optimal_path_TSPTW(self, memo: dict, n: int) -> [[int], float]:
        ...
        if len(full_path_memo) == 0:
            ...
            optimal_path_departure_times = [max(optimal_path_arrival_times[i] + self.service_times[i], start1[i]) for i
                                            in range(len(self.distances_array))]
            self.optimal_path = optimal_path
            self.optimal_cost = optimal_cost
            self.df1 = pd.DataFrame({'opt path': optimal_path, 'start': start1, 'arrival': optimal_path_arrival_times,
                                     'departure': optimal_path_departure_times, 'end': end1})
            print("time check:")
            print(self.df1)
    ...
    def Dumas_TSPTW_Solve(self, T1, T2, T3):
        ...
        while self.queue.qsize() != 0:
            ...
            for self.new_last_point in to_visit:
                ...
                self.new_time = max(self.prev_time, self.start_times[self.prev_last_point]) + self.service_times[
                    self.prev_last_point] + self.travel_times_array[self.prev_last_point][self.new_last_point]
    ...
    def duplicate_label_check_VRP_update2(self):
        ...
        if ((
            ...
            if tuple(self.sorted_nd) == tuple(costcheck):
                dup_lab_check = False
                logging.info(
                    f"the label {(tuple(self.new_visited), tuple(self.sorted_nlp), tuple(self.sorted_nt), self.key_version)} is a duplicate")
            ...
            dup_lab_check = True
            logging.info(
                f"the label {(tuple(self.new_visited), tuple(self.sorted_nlp), tuple(self.sorted_nt), self.key_version)} HAS NOT already been encountered")
    ...
    def duplicate_label_check_VRP_update3(self):
        ...
        dup_lab2 = {k: v for k, v in self.rejected_labels.items() if
                    k[0] == self.new_visited and k[1] == self.sorted_nlp}
        if len(dup_lab1) + len(dup_lab2) == 0:
            logging.info(
                f"the label {(tuple(self.new_visited), tuple(self.sorted_nlp), tuple(self.sorted_nt))} HAS NOT already been encountered")
        else:
    ...
    def VRP_time_window_check(self):
        ...
        if False in self.TW:
            TW_test = False
            self.rejected_labels[
                (tuple(self.new_visited), tuple(self.sorted_nlp), tuple(self.sorted_nt), self.key_version)] = list(
                self.sorted_nd)
        else:
            TW_test = True
        ...
        if len(remaining_jobs) == 0:
            ...
            for i in self.sorted_nlp:
                for j in remaining_jobs:
                    logging.debug(
                        f"self.start_times[{i}] = {self.start_times[i]} and self.end_times[{j}] = {self.end_times[j]}")
            ...
            while len(vehicles_to_check) != 0 and len(unreachable_points) != 0:
                ...
                for j in unreachable_points:
                    if self.first[vehicle_to_check] <= self.LDT_array[j][self.sorted_nlp[vehicle_to_check]]:
                        logging.debug(
                            f"first = {self.first[vehicle_to_check]} <= LDT({j},{self.sorted_nlp[vehicle_to_check]}) = {self.LDT_array[j][self.sorted_nlp[vehicle_to_check]]}")
    ...
    def VRP_dominance_test_update4(self):
        ...
        if dom_lab == None:  # no labels exist to dominate the current label
            ...
            logging.debug(
                f"dom_check_1_time = {dom_check_1_time}, dom_check_1_dist = {dom_check_1_dist}, dom_check_2_time = {dom_check_2_time}, dom_check_2_dist= {dom_check_2_dist}")
            dom_check_1 = False
            for i in range(len(dom_lab)):
            ...
            if dom_check_1:
                logging.info(
                    f"the current label ({self.new_visited, self.sorted_nlp, self.sorted_nt}) with costs {self.sorted_nd} is dominated by the existing label ({self.new_visited, self.sorted_nlp, self.prefix_label_times[dominator]}) with costs {self.prefix_label_dist[dominator]} so no new label is added.")
            # existing labels are deleted if dom_check_1 is true because the current label dominates those labels, the current label will be added later
                ...
                if len(dominated) != 0:
                    ...
                    for i in dominated:
                        logging.info(
                            f"the current label ({self.new_visited, self.sorted_nlp, self.sorted_nt}) with costs {self.sorted_nd} dominates the existing label {i} with costs {self.memo[i][0]} so the existing label is deleted.")
                        del self.memo[i]
                        # self.queue.remove(i)
                        temp.remove(i)
                    logging.debug(f"temp = {temp}")
                    ...
                    logging.info(
                        f"the current label ({self.new_visited, self.sorted_nlp, self.sorted_nt, self.key_version}) with costs {self.sorted_nd} DOES NOT dominate the existing label ({self.new_visited, self.sorted_nlp, self.prefix_label_times[i]}) with costs {self.prefix_label_dist[i]} so the existing label is not deleted.")
            ...
            if not dom_check_1:
                ...
                if (tuple(self.new_visited), tuple(self.sorted_nlp)) in self.prefix_memo:
                    ...
                    logging.debug(f"prefix_memo = {self.prefix_memo}")
                logging.info(
                    f"the current label ({self.new_visited, self.sorted_nlp, self.sorted_nt, self.key_version}) with costs {self.sorted_nd} is a duplicate label to some existing label, but with different values. So the current label is updated and added.")
        return
    ...
    def prefix_search2(self):
        ...
        if self.prefix_labels != None:
            ...
            self.prefix_label_dist = None
        logging.debug(
            f"prefix time = {self.prefix_label_times}, prefix dist = {self.prefix_label_dist}, prefix labels = {self.prefix_labels}")
        return
    ...
    def VRP_Solve(self, DUP, TW, SJA, DOM, T1, T2):
        ...
        while self.queue.qsize() != 0:
            ...
            while self.memo.get((tuple(self.prev_visited), tuple(self.prev_last_point), tuple(self.prev_time),
                ...
                self.prev_visited, self.prev_last_point, self.prev_time, self.prev_key_version = self.queue.get()
            logging.debug(
                f"extending label {self.prev_visited}, {self.prev_last_point}, {self.prev_time}, {self.prev_key_version}")
            if len(self.prev_visited) != counter:
                print(
                    f"progress check: {len(self.prev_visited)} cities of {self.number_of_jobs} cities visited, the running time is {time.time() - self.t}, and {len(self.memo)} labels have been created")
                counter = counter + 1
            self.prev_dist, _, _, self.vehicle_order, _ = self.memo[
                (tuple(self.prev_visited), tuple(self.prev_last_point), tuple(self.prev_time), self.prev_key_version)]
            ...
            for i in range(self.number_of_vehicles):
                ...
                for self.new_last_point[i] in to_visit:
                    ...
                    self.new_dist[i] = float(np.round(
                        self.prev_dist[i] + self.distances_array[self.prev_last_point[i]][self.new_last_point[i]], 2))
                    self.new_time[i] = float(np.round(
                        max(self.prev_time[i], self.start_times[self.prev_last_point[i]]) + self.service_times[
                            self.prev_last_point[i]] + self.travel_times_array[self.prev_last_point[i]][
                            self.new_last_point[i]], 2))
                    logging.info(
                        f"checking the new label ({self.new_visited},{self.new_last_point},{self.new_time}) with distances {self.new_dist}")
                    self.sorted_nlp, self.sorted_nt, self.sorted_nd, self.sorted_vo = zip(
                        *sorted(zip(self.new_last_point, self.new_time, self.new_dist, self.vehicle_order)))
                    self.prefix_search2()
                    if DUP:
                    ...
                    if DOM:
                        ...
                        if (tuple(self.new_visited), tuple(self.sorted_nlp), tuple(self.sorted_nt),
                            self.key_version) in self.memo:
                            while (tuple(self.new_visited), tuple(self.sorted_nlp), tuple(self.sorted_nt),
                                   self.key_version) in self.memo:
                                self.key_version = self.key_version + 1
                            ...
                            self.queue.put((tuple(self.new_visited), tuple(self.sorted_nlp), tuple(self.sorted_nt),
                                            self.key_version))
                            temp = self.prefix_memo[(tuple(self.new_visited), tuple(self.sorted_nlp))]
                            temp.append((tuple(self.new_visited), tuple(self.sorted_nlp), tuple(self.sorted_nt),
                                         self.key_version))
                            self.prefix_memo[(tuple(self.new_visited), tuple(self.sorted_nlp))] = temp
                        else:
                            self.memo[(tuple(self.new_visited), tuple(self.sorted_nlp), tuple(self.sorted_nt),
                            ...
                            if (tuple(self.new_visited), tuple(self.sorted_nlp)) in self.prefix_memo:
                                ...
                                self.prefix_memo[(tuple(self.new_visited), tuple(self.sorted_nlp))] = [(tuple(
                                    self.new_visited), tuple(self.sorted_nlp), tuple(self.sorted_nt), self.key_version)]
    ...
    def Solver(self, read_in_data, data, random_data, instances, timeframe, locationframe, servicetime, serviceframe,
        ...
        if random_data == True:
            self.random_data_generator(instances, timeframe, locationframe, servicetime, serviceframe,
                                       travel_times_multiplier, save_name)
            print(self.df)
        else:
...
for i in range(len(names15)):
    for j in {0, 1, 2, 3}:
        print(f"data set = {tight_windows[i]}")
        print(f"TW = True")
        print(f"DUP = True")
        print(f"SJA = True")
        print(f"DOM = True")
        ...
        pr = cProfile.Profile()
        pr.enable()
        a.Solver(read_in_data=True, data=tight_windows[i], random_data=False, instances=7, timeframe=2000, locationframe=100,
                 servicetime=True, serviceframe=25, travel_times_multiplier=1, save_name=big_names[i], DUP=True,
                 TW=True, T1=t1[j], T2=t2[j], T3=False, SJA=True, DOM=True)
        print(
            f"###COMPLETE_RESULTS:, {tight_windows[i]}, T1 = {t1[j]}, T2 = {t2[j]}, {a.run_time}, {a.optimal_cost}, {a.optimal_path}, {len(a.memo)}")
        pr.disable()
        s = io.StringIO()
